...
def insert(root, k):
    ...
    if root is None:
        ...
        # parent of the Node containing k.
        while (k < r.key and r.left is not None) or \
                (k > r.key and r.right is not None):
            if k < r.key:
                r = r.left
            else:
                r = r.right
...
def contains(root, k):
    ...
    r = root
    while r is not None and k != r.key:
        if k < r.key:
            r = r.left
        else:
            r = r.right
...
def traverse(root, func, container):
    ...
    # As long as container isn't empty, remove the next Node, apply func to
    # its key, and insert the left and right child Nodes into container (if
    # the Node has any children).
    while not container.is_empty():
        node = container.remove()
        func(node.key)
        if node.left is not None:
            container.insert(node.left)
        if node.right is not None:
            container.insert(node.right)
